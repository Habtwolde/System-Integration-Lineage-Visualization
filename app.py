        ...
        if st.button("Submit"):
            ...
            if not filtered_df.empty:
                ...
                for _, row in filtered_df.iterrows():
                    ...
                    sankey_links.append({"source": row['source_idx'], "target": row['technology_idx'], "value": 5, "color": f"rgba({row['source_idx']*40 % 255}, {row['technology_idx']*80 % 255}, {row['target_idx']*120 % 255}, 0.6)"})
                    # Link from Technology to System To
                    sankey_links.append({"source": row['technology_idx'], "target": row['target_idx'], "value": 5, "color": f"rgba({row['source_idx']*40 % 255}, {row['technology_idx']*80 % 255}, {row['target_idx']*120 % 255}, 0.6)"})

                # Descriptive labels for nodes
                node_labels = []
